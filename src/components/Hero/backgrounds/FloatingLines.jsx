...
export default function FloatingLines({
  ...
  bendRadius = 5.0,
  bendStrength = -0.5,
  mouseDamping: _mouseDamping = 0.05,
  parallax = true,
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event) => {
      ...
      const mx = (x / Math.max(rect.width, 1)) * renderer.domElement.width;
      const my =
        (1 - y / Math.max(rect.height, 1)) * renderer.domElement.height;

      currentMouseRef.current.set(mx, my);
    ...
    // Window listener — avoids getBoundingClientRect thrash only on canvas,
    // and still works while content sits above the bg.
    if (interactive) {
    ...
    // Mount once — unstable array props from parents were remounting on scroll.
